...
class SyncContent(object):
    ...
    def update_content(self, videos):
        for video in videos:
            ...
            for vf in video_files:
                is_success = (
                    self.update_videofile(vf, video['id']) and
                    is_success
                )
    ...
    def update_video_content(self, videos, video_id):
        """
        Возвращает True, если указанный VIDEO_ID обработан
        """
        for video in videos:
    ...
    def load_video(self):
        ...
        if not updated and response['count'] > limit:
            pages = self.get_pages(limit, response['count'])
            for i in range(2, pages+1):
                response = self.api.video_list(page=i, limit=limit)
                updated = self.update_video_content(
                    response['videos'],
                    VIDEO_ID,
                )
